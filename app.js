import { firebaseConfig } from './firebase-config.js?v=9.5';
...
async function enviarPushMVL(tipo,titulo,mensagem,destinatarios=[]){
  try{
    ...
    const resposta=await fetch(MVL_PUSH_ENDPOINT,{method:'POST',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify({idToken,tipo,titulo,mensagem,destinatarios,url:'https://robertjuniorrj85-ux.github.io/App-MVL/'})});
...
async function renderCharts(editItem=null){
  showSectionHeader('Cifras','Cadastre cada cifra uma vez e vincule ao cantor ou deixe como Padrão / Todos.');await refreshCaches();adminPanel.classList.toggle('hide',!isAdmin);
  if(isAdmin){const d=editItem||{};$('form-title').textContent=editItem?'Editar cifra':'Nova cifra';editingId=editItem?.id||null;cancelEditBtn.classList.toggle('hide',!editItem);saveBtn.textContent=editItem?'Atualizar':'Salvar';
  ...
  chartsCache.slice().sort((a,b)=>(a.musica||'').localeCompare(b.musica||'')).forEach(c=>{const song=songById(c.musicaId)||{};const card=document.createElement('div');card.className='item-card';card.innerHTML=`<h3>🎼 ${escapeHtml(song.titulo||c.musica||'Cifra')}</h3><div class="item-meta"><b>Tom:</b> ${escapeHtml(c.tom||'—')}</div><div class="item-meta">${c.cantorId?'🎤 '+escapeHtml(c.cantorNome||userName(c.cantorId)):'🌐 Padrão / Todos'}</div>${c.observacoes?`<div class="item-meta">${escapeHtml(c.observacoes)}</div>`:''}<div class="item-actions">${safeUrl(c.link)?`<a class="link-btn" href="${escapeAttr(c.link)}" target="_blank" rel="noopener">🎼 Abrir Cifra</a>`:''}${isAdmin?'<button class="edit-btn">Editar</button><button class="delete-btn">Excluir</button>':''}</div>`;if(isAdmin){card.querySelector('.edit-btn').onclick=()=>{renderCharts(c);window.scrollTo({top:0,behavior:'smooth'});};card.querySelector('.delete-btn').onclick=async()=>{if(confirm('Excluir esta cifra?')){await deleteDoc(doc(db,'cifras',c.id));await refreshCaches();renderCharts();}};}listEl.appendChild(card);});
}
...
async function renderSongs(editItem=null){
  ...
  for(const s of songsCache){const card=document.createElement('div');card.className='item-card';const linked=chartsCache.filter(c=>chartSongId(c)===s.id).map(c=>`${c.cantorId?(c.cantorNome||userName(c.cantorId)):'Padrão'} — ${c.tom||'sem tom'}`).join(' • ');card.innerHTML=`<h3>🎵 ${escapeHtml(s.titulo||'Sem título')}</h3><div class="item-meta">${escapeHtml(s.artistaReferencia||s.artista||'')}</div><div class="item-meta"><b>Tom original:</b> ${escapeHtml(s.tomOriginal||s.tom||'—')}</div>${linked?`<div class="item-meta"><b>Cifras/Tons:</b> ${escapeHtml(linked)}</div>`:''}<div class="item-actions">${safeUrl(s.ouvirLink||s.link)?`<a class="link-btn" href="${escapeAttr(s.ouvirLink||s.link)}" target="_blank" rel="noopener">▶️ Ouvir</a>`:''}${safeUrl(s.multitrackLink)?`<a class="link-btn" href="${escapeAttr(s.multitrackLink)}" target="_blank" rel="noopener">🎚️ Multitrack</a>`:''}${isAdmin?'<button class="edit-btn">Editar</button><button class="delete-btn">Excluir</button>':''}</div>`;
